...
from aioquic.tls import SIGNATURE_ALGORITHMS, SERVER_CONTEXT_STRING,CLIENT_CONTEXT_STRING,signature_algorithm_params,SignatureAlgorithm,load_pem_private_key
class dhke:
    ...
    def handshake_traffic_computation(_cipher_suite : CipherSuite = 0x1302) :
        SessionInstance.get_instance().server_handshake_traffic_secret =  dhke.handshake_secret(cipher_suite = _cipher_suite,shared_key = SessionInstance.get_instance().shared_key, lable = b"s hs traffic",isClient = False)
        ...
        SessionInstance.get_instance().client_handshake_traffic_secret =  dhke.handshake_secret(cipher_suite = _cipher_suite,shared_key = SessionInstance.get_instance().shared_key, lable = b"c hs traffic",isClient = True)
    ...
    def appliction_traffic_computation(_cipher_suite : CipherSuite = 0x1302) :
        SessionInstance.get_instance().server_appliction_traffic_secret =  dhke.ap_secret(cipher_suite = _cipher_suite, handshake_secret = SessionInstance.get_instance().server_handshake_secret , label=  b"s ap traffic")
        ...
        SessionInstance.get_instance().client_appliction_traffic_secret =  dhke.ap_secret(cipher_suite = _cipher_suite, handshake_secret = SessionInstance.get_instance().client_handshake_secret , label=  b"c ap traffic")
    ...
    def get_certificate_verify_signature():
        with open("ssl_key.pem", "rb") as fp:
            certificate_private_key = load_pem_private_key(
                        fp.read(),
                        password=None
                    )
